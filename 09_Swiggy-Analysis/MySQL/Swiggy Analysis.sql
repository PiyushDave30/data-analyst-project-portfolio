...
from swiggy;
/* Insight: avg price ~₹220 with a wide range (₹0.95–₹8,000) High-end outliers likely represent bulk/catering orders. */
...
from swiggy;
/* Insight: avg rating of ~4.0 is healthy. A large number of unrated dishes suggests many new or low-traffic menu items. */
...
/* Insight: Veg dishes dominate at ~71.5% (1,40,604 orders) vs Non-Veg at ~28.5% (56,826 orders) — reflecting India's strong vegetarian food culture. */
...
select city, round(avg(`price (INR)`), 2) as avg_price, count(*) as total_orders from swiggy group by city having avg_price > 300 order by avg_price desc;
/* Insight: Only 2 cities clear the ₹300 avg price mark — Panaji (₹306) and Lucknow (₹305). Most Indian cities including metros like Mumbai and Delhi fall below ₹300 avg, showing price sensitivity across the platform.*/

-- Find restaurants that have more than 50 dishes on their menu. Show restaurant name, city, and dish count.
select `restaurant name`,city,count(distinct `dish name`) as total_dishes from swiggy group by `restaurant name`, city having total_dishes > 50 order by total_dishes desc;
...
select case when `day` in ("Sat","Sun") then "Weekend" else "Weekday" end as Day_Type, count(*) as "Total Orders", concat(round(count(*) * 100.0 / sum(count(*)) over(),2)," %") as Percentage
from swiggy group by day_type;
/* Insight:  Weekdays account for 70.92% of orders (1,40,018) vs weekends at only 29.08% (57,412) — OPPOSITE of what was assumed. Swiggy's core audience is actually need/habit driven on weekdays, not just leisure
weekend ordering. Weekdays = 5 days, weekends = 2 days, yet weekdays dominate even proportionally (~14% per day vs ~14.5% per day — nearly equal) */

-- *****************************************************************************************************************************************************
-- *****************************************************************************************************************************************************

-- Q1. Create a price category label for every dish: Budget → below ₹150 Mid-Range → ₹150 to ₹400 Premium → ₹400 to ₹800 Luxury → above ₹800 Show dish name, price, and the label. Limit 20.
...
SELECT 
    CASE 
        ...
        ELSE 'Luxury'
    END  AS price_category,
    COUNT(*)  AS total_dishes, CONCAT(ROUND(COUNT(*) * 100.0 / SUM(COUNT(*)) OVER(), 2), ' %') AS percentage
...
FROM swiggy GROUP BY `restaurant name`, city HAVING SUM(`rating count`) >= 50;
